...
- \( \delta : Q \times \Sigma \to Q \) – the **transition function** (deterministic)
...
### 2.2.3 ε‑NFA and ε‑closure

An **ε‑NFA** allows transitions on the empty string ε, meaning the machine can change state without consuming any input symbol.

**Formal definition:** \( \delta : Q \times (\Sigma \cup \{\varepsilon\}) \to 2^Q \).

**ε‑closure** of a set of states \( S \) is the set of all states reachable from \( S \) by following zero or more ε‑transitions. Denoted \( \text{ECLOSE}(S) \).

**Example ε‑NFA:** Accepts strings over `{a,b}` where the number of `a` is even (can be built with ε‑transitions to model parity). More standard: ε‑NFA for the regular expression `a*`:
...
But ε‑transitions are typically used to glue smaller automata.

**Algorithm for ε‑closure:**
```
ECLOSE(S):
    closure = S
    stack = S (as a list)
...
## 2.3 Equivalence of DFA, NFA, and ε‑NFA
...
- Every ε‑NFA can be converted to an equivalent NFA (by removing ε‑transitions via ε‑closure) and then to a DFA.

**Proof outline (ε‑NFA → NFA):**  
Given ε‑NFA \( E = (Q_E, \Sigma, \delta_E, q_0, F_E) \), construct NFA \( N = (Q_E, \Sigma, \delta_N, q_0, F_N) \) where:

- \( \delta_N(q, a) = \text{ECLOSE}( \bigcup_{p \in \delta_E(q, a)} \text{ECLOSE}(p) ) \) for \( a \in \Sigma \)
- \( F_N = \{ q \mid \text{ECLOSE}(q) \cap F_E \neq \emptyset \} \)

This NFA accepts the same language without ε‑transitions.

**Equivalence theorem:** For any ε‑NFA, there exists a DFA recognising the same language, and vice versa.
...
- Start state: \( q_{D0} = \{ q_0 \} \) (or ε‑closure for ε‑NFA)
...
| Model | Transition | Determinism | ε‑moves | Power |
|-------|-----------|-------------|---------|-------|
| DFA | δ: Q×Σ → Q | Yes | No | Regular languages |
| NFA | δ: Q×Σ → 2^Q | No | No | Regular languages |
| ε‑NFA | δ: Q×(Σ∪{ε})→2^Q | No | Yes | Regular languages |

Key takeaways:
- DFA, NFA, ε‑NFA are **equally expressive**.
- Subset construction converts NFA/ε‑NFA to DFA (exponential blow‑up in worst case).
