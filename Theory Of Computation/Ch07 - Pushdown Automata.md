...
| $\delta$ | **transition function**: $Q \times (\Sigma \cup \{\varepsilon\}) \times \Gamma \to \mathcal{P}(Q \times \Gamma^*)$ |
...
In state $q$, reading input $a$ (or $\varepsilon$ for epsilon-transition), with $X$ on top of stack, the PDA may move to state $p$ and replace $X$ with the string $\gamma$ (push/pop operations encoded in $\gamma$).

- **Push** $Y$: $\gamma = YX$ (adds $Y$ on top)
- **Pop** $X$: $\gamma = \varepsilon$
- **No change**: $\gamma = X$
...
**One move** $\vdash$ is defined:
$(q, a w, X \gamma) \vdash (p, w, \beta \gamma)$ if $(p, \beta) \in \delta(q, a, X)$ (where $a \in \Sigma \cup \{\varepsilon\}$).

We write $\vdash^*$ for zero or more moves.

**Example sequence** for $L = \{a^n b^n\}$ (pop a for each b):

$$
(q_0, aabb, Z_0) \vdash (q_0, abb, A Z_0) \vdash (q_0, bb, AA Z_0) \vdash (q_1, b, A Z_0) \vdash (q_1, \varepsilon, Z_0)
$$
...
Given a CFG $G = (V, \Sigma, R, S)$ in **Greibach normal form** (all productions $A \to a\alpha$ where $a \in \Sigma$, $\alpha \in V^*$), we build a PDA with **one state**:

$$
M = (\{q\}, \Sigma, V, \delta, q, S, \emptyset)
$$
Transitions:
- For each production $A \to a B_1 B_2 \cdots B_k$:
  $\delta(q, a, A) \ni (q, B_1 B_2 \cdots B_k)$
- For each terminal $a$ (if a production $A \to a$):
  $\delta(q, a, A) \ni (q, \varepsilon)$
...
**Example:** $S \to a S b \mid \varepsilon$ converted to Greibach:
$S \to a S B$, $B \to b$, plus $S \to \varepsilon$.
PDA transitions:
...
1. **Start symbol:** $S \to [q_0 Z_0 q]$ for every $q \in Q$.
2. For each transition $(r, Y_1 Y_2 \cdots Y_k) \in \delta(p, a, X)$ (where $k \ge 0$):
   - If $k = 0$ (pop): add $[p X r] \to a$
   - If $k \ge 1$: for all sequences of states $q_1, q_2, \dots, q_k \in Q$, add
     $$
     [p X q_k] \to a\, [r Y_1 q_1] [q_1 Y_2 q_2] \cdots [q_{k-1} Y_k q_k]
     $$
3. For any state $p$ and stack symbol $X$: add $[p X p] \to \varepsilon$? (Actually this is not needed; the construction automatically generates derivations that exactly consume input.)

**Simplified algorithm** (often taught):
Introduce nonterminal $(p, X, q)$ meaning "pop X while going from p to q". For each transition of the form $\delta(p, a, X) \ni (r, Y_1 \dots Y_m)$ and for every choice of states $q_1, \dots, q_m$, create a production
$(p, X, q_m) \to a (r, Y_1, q_1) \dots (q_{m-1}, Y_m, q_m)$.
...
#### Example 1: $L = \{a^n b^n \mid n \ge 0\}$
...
#### Example 2: $L = \{w w^R \mid w \in \{a,b\}^*\}$ (even-length palindromes)
...
#### Example 3: $L = \{a^i b^j c^k \mid i = j \text{ or } j = k\}$
