...
- **TCP / UDP**: IETF RFC 793 (TCP) and RFC 768 (UDP). Also: Stevens, W. R. (1994). *TCP/IP Illustrated, Volume 1*. Addison-Wesley.
- **OSI vs TCP/IP**: Zimmermann, H. (1980). "OSI Reference Model – The ISO Model of Architecture for Open Systems Interconnection". *IEEE Transactions on Communications*. Also: RFC 1122 (Requirements for Internet Hosts – Communication Layers).
- **HTTP vs HTTPS**: IETF RFC 9110 (HTTP Semantics) and RFC 8446 (TLS 1.3). Also: "HTTPS Explained" – Let's Encrypt documentation (letsencrypt.org).
- **DNS**: IETF RFC 1034 and 1035 (Domain Names – Concepts and Facilities). Also: Liu, C. & Albitz, P. (2006). *DNS and BIND*. O'Reilly Media.
- **3-Way Handshake / 4-Way Termination**: IETF RFC 793 (TCP). Also: Kurose, J. & Ross, K. (2021). *Computer Networking: A Top-Down Approach*. Pearson.
- **NAT**: IETF RFC 2663 (IP Network Address Translator Terminology) and RFC 3022 (Traditional NAT). Also: "NAT Traversal Techniques" – IETF RFC 5389 (STUN), RFC 5766 (TURN), RFC 8445 (ICE).
- **Hub, Switch, Router**: IEEE 802.3 (Ethernet), IEEE 802.1Q (VLANs), and IETF RFC 1812 (Requirements for IP Routers). Also: Tanenbaum, A. & Wetherall, D. (2010). *Computer Networks*. Prentice Hall.
